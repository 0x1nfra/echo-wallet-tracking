...
- [ ] **DATA-01**: System persists wallet registry, swap history, metrics, signals, and removal log to SQLite
- [ ] **DATA-02**: Database uses WAL mode to allow concurrent reads during monitoring loop writes
...
- [ ] **SCOR-01**: System calculates wallet metrics: win rate, realized PnL in SOL, risk-adjusted return (Sharpe-like ratio), max drawdown, and recency score
- [ ] **SCOR-02**: System produces a 0-100 wallet score weighted: risk-adjusted return (40%), win rate (20%), consistency and recency (20%), activity health (20%)
...
- [ ] **SGNL-01**: System computes a per-token signal score (0-100) based on: count of smart wallets holding, buy velocity in last 1h, exit pressure from sells, and PnL-weighted holder score
- [ ] **SGNL-02**: System updates all token signals after each monitoring cycle completes
- [ ] **SGNL-03**: System discounts a token's signal score when its holders appear coordinated (share a common funding source)

### Web Dashboard

- [ ] **DASH-01**: User can view a live token signal feed sorted by signal score
...
- [ ] **TGRM-01**: User receives a Telegram alert when a token signal score crosses a configured threshold
- [ ] **TGRM-02**: System deduplicates alerts — no more than 1 alert per token per 2 hours
- [ ] **TGRM-03**: User can query the bot with /status, /top (top current signals), and /wallet <address>
...
| TGRM-01 | Phase 7 | Pending |
| TGRM-02 | Phase 7 | Pending |
| TGRM-03 | Phase 7 | Pending |
