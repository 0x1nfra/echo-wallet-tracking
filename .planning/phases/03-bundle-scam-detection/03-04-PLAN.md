...
must_haves:
  truths:
    ...
    - "wallet flag <address> manually force-promotes a wallet's detection tier (force-promote path), accepts --detector and --tier options, prompts confirmation before writing, inserts a wallet_flag row with cleared=false, recomputes wallets.detection_status"
  ...
  artifacts:
    ...
    - path: "src/commands/wallet.ts"
      provides: "wallet list (two sections), wallet review, wallet clear-flag, wallet flag commands"
      contains: "clear-flag"
      exports: ["wallet flag", "wallet clear-flag", "wallet review", "wallet list"]
  key_links:
...
<task type="auto">
  ...
  <action>
    ...
    **2. Update src/commands/wallet.ts:**
    ...
    e) Add `wallet flag` command (force-promote — user can manually escalate any wallet to a chosen tier):
    ```typescript
    wallet
  ...
  <verify>
    ...
    4. `echo wallet flag --help` — shows command help with --detector and --tier options listed.
    5. `echo wallet flag <nonexistent-address>` — exits with error "Wallet not found" message.
    6. `npx tsc --noEmit` — zero TypeScript errors.
    7. Add a wallet and confirm detection runs: `echo wallet add <address>` should log "Wallet import complete" and then silently run detection (no error output).
  </verify>
  <done>
    importWalletHistory calls runDetection(address) after history_complete=true is set. wallet list shows separate Clean Wallets and Flagged Wallets sections. wallet review lists flagged wallets with per-flag evidence tables. wallet clear-flag shows evidence, prompts confirmation, clears flags, raises threshold_multiplier (capped at MAX_THRESHOLD_MULTIPLIER), and recomputes wallet detection_status from remaining flags. wallet flag accepts --detector and --tier, displays intended write, prompts confirmation, inserts a wallet_flag row with cleared=false, and recomputes wallets.detection_status via computeOverallStatus. getEligibleWallets() returns only confirmed_passing wallets.
  </done>
</task>
...
6. `echo wallet flag --help` — shows command help with --detector and --tier options
7. Verify detection gate: `npx tsx -e "import { getEligibleWallets } from './src/detection/engine.js'; console.log(getEligibleWallets())"` — returns array (empty on fresh DB, correct)
...
- wallet flag: accepts --detector and --tier, displays write preview, prompts confirmation, inserts wallet_flag row (cleared=false), recomputes detection_status — escalation path per user decision
